...
| **Framework-internal system-mode selectors**                    | `QRY_Builder` and `DML_Builder` enforce FLS/CRUD by default. About three-quarters of the 44 selector classes opt back into system-mode via a documented per-selector hook (e.g., configuration reads, Chain Monitor aggregates). The opt-out is auditable in source, enforced by a CI-blocking scanner rule, and rollback-safe via metadata kill-switches. When you extend `SEL_Base` you inherit FLS-enforced reads by default; opting out requires an explicit override.                                                                                                                                                                                                                                                                                                            |
