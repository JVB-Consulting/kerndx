...
See also: [Adoption](Strategic%20Guide%20-%20Adoption.md) | [Operations](Strategic%20Guide%20-%20Operations.md) | [Risks](Strategic%20Guide%20-%20Risks.md) | [Glossary](Strategic%20Guide%20-%20Glossary.md) | [Personas](Strategic%20Guide%20-%20Personas.md)
...
| Data masking | `MaskingRule__mdt` + `MaskingTarget__mdt` | Runtime field redaction for any SObject or platform event before the record is persisted. Four masking modes (regex, JSON-key, exact-match, credit-card with Luhn validation) plus 14 built-in masking rules. Default ship set wires two active rules to four package objects through eight `MaskingTarget__mdt` records; admins extend coverage to their own objects by adding more `MaskingTarget__mdt` records. Rules can be scoped via `MinInputLength__c` and `ApplicableFieldTypes__c`. |
...
In a modular stack, these integrations require explicit instrumentation at each boundary. However, this automatic coherence requires all code to flow through the framework — increasing coupling to a single vendor's conventions and release cycle. Coherence is best understood as an entropy management mechanism (see [Operational Entropy and Maintainability in Multi-Team Environments](Strategic%20Guide%20-%20Adoption.md#operational-entropy-and-maintainability-in-multi-team-environments)), not an inherent quality advantage.
...
**Integration burden — amortized across capabilities.** KernDX bundles the 16 framework areas into one managed package with one namespace, one upgrade cadence, one security-review surface, and one bypass-audit signal. Subscribers picking individual open-source pods to assemble equivalent coverage accept three carrying costs that the bundled framework amortizes: operational integration cost (configuring multiple pods to coexist — namespace alignment, settings hierarchies, cross-pod bypass coordination), security-review carrying cost (each pod is its own review surface per upgrade), and upgrade / maintenance carrying cost (independent release cadences across the pod set). Because every capability ships against the same shared framework infrastructure (trigger surface, logging, masking, bypass-audit signal), subscribers pay the integration cost once at framework adoption rather than per pod for each capability added. The the Category considerations — integration burden analysis section lays out the category-level framing; this section explains the architectural choice that makes the amortization possible.
...
| **v1.0 — limited external production deployment at the snapshot date** | First validated build packaged; testing-hardened (100% per-file Apex coverage gate, 95% LWC, 471 anon-Apex assertions in the subscriber e2e harness); one external client engagement in active use at the snapshot date, broader subscriber adoption on the public-release rollout path. See [Adoption Signal Profile](#adoption-signal-profile) for what closes this gap, and [Metrics — Activity Snapshot](Strategic%20Guide%20-%20Metrics.md#activity-snapshot) for the current build identifier. |
| **Full package deployment** | Managed package installs all 351 classes. Unused code is genuinely inert — exempt from 6 MB limit, invisible to subscriber PMD/coverage, zero governor impact. The cost is namespace prefix on references and a single (larger) upgrade cycle, not deployed footprint. |
| **Namespace verbosity** | Subscriber code requires namespace prefix. Modern IDEs auto-complete this. |
| **No CDC channel binding** | KernDX dispatches Apex triggers and flow trigger-actions on standard DML events but does not provide CDC (Change Data Capture) channel binding. `taf` supports CDC via [`TriggerActionFlowChangeEvent`](https://github.com/mitchspano/apex-trigger-actions-framework/blob/main/trigger-actions-framework/main/default/classes/TriggerActionFlowChangeEvent.cls) — the channel registration itself is **subscriber-authored Apex** (a trigger on the change-event SObject like `AccountChangeEvent` calling `new MetadataTriggerHandler().run()`); the per-flow routing is then declarative. If CDC-driven dispatch is a hard requirement, `taf` is the cleaner fit because the metadata + flow-input plumbing already exists; otherwise both ship the same declarative trigger registration on substance. KernDX adds declarative `LogAndContinue` / `BlockDml` error strategies, deploy-time flow validation, and a `TST_InvokeFlowMock` test harness; KernDX's per-invocation single-record shape reflects the `Invocable.Action.setInvocations` constraint at API 66.0. |
...
| **Framework-internal system-mode selectors** | `QRY_Builder` and `DML_Builder` enforce FLS/CRUD by default. About three-quarters of the 42 selector classes opt back into system-mode via a documented per-selector hook (e.g., configuration reads, Chain Monitor aggregates). The opt-out is auditable in source, enforced by a CI-blocking scanner rule, and rollback-safe via metadata kill-switches. Subscribers who extend `SEL_Base` inherit FLS-enforced reads by default; opting out requires an explicit override. |
...
| Module | Time to First Use | What You Learn |
|--------|-------------------|----------------|
| **TST_Builder** | 10 minutes | `.of(SObjectType).build()` — one line of code |
| **QRY_Builder** | 1-2 hours | Fluent query syntax, replaces inline SOQL |
| **DML_Builder** | 1-2 hours | Transactional DML with parent-child chaining |
| **LOG_Builder** | 1-2 hours | `.build().error(e).emitAt('Class.method')` — one line |
| **UTIL_Cache** | 30 minutes | `.put()`, `.get()`, `.contains()` — standard cache API |
| **TRG_\* + TRG_Dispatcher** | 1-2 days | Trigger actions, metadata configuration, bypass patterns |
| **SEL_Base selectors** | 2-4 hours | Extend `SEL_Base`, define `getFields()`, use inherited methods |
| **API_Outbound** | 1-2 days | Outbound web service pattern with mock, retry, circuit breaker |
| **Full framework** | 1-2 weeks (incremental) | All modules, with each building on the previous |
...
When a new developer joins a team using KernDX, the path from "I've never seen this" to "I built my first trigger action" is documented. DORA research (2022) found that high-quality documentation dramatically amplifies the performance lift of technical practices — see [Build vs Buy: Cost Considerations](Strategic%20Guide%20-%20Adoption.md#build-vs-buy-cost-considerations) for the qualitative cost framing this implies.
...
| **Documentation** | 26 developer documents (14 guides, 12 fast starts), 263 API references | Per-library documentation sites |
...
| **Selector Framework** | 42 | Object-specific query layer with default fields (selector pattern in the Query Builder family — typed subselects, semi-join + anti-join builder methods, unbounded subselect chain depth) | `SEL_Base` |
...
| **Logging** | 6 | Async structured logging via platform events | `LOG_Builder` |
...
| **Security** | 2 | FLS/CRUD enforced by default on reads AND writes; encryption default-on; sharing control; runtime field masking | `QRY_Builder`, `DML_Builder`, `UTIL_SessionEncryption` |
...
| **LWC** | 52 | Component library with five built-in modules and registry-dispatch activation (only Apex framework surveyed with a component-base + module-mixin composition surface) | `ComponentBuilder` |
...
KernDX's configuration-over-code approach is backed by 14 custom metadata types organized in a clear hierarchy:
...
│ (per handler)   │  │ ApiMock         │  │ (per formula)        │
└─────────────────┘  │ MaskingRule     │  └──────────────────────┘
                     │ MaskingTarget   │
                     └─────────────────┘
         ┌─────────────────────┼──────────────────────┐
...
| Metric | Value |
|--------|-------|
| Apex classes | 351 (183 production, 168 test) |
| Selector classes | 42 (extending `SEL_Base`) |
| Flow invocable classes | 16 (`FLOW_*`) |
| Test methods | ~5,711 (~3,443 Apex + ~2,268 Jest) |
...
| Platform events | 1 |
| Pre-built metadata records | 53 |
| Source API version | 66.0 (pinned in `sfdx-project.json`) |
...
| Documentation | 26 developer documents (14 guides, 12 fast starts), 263 API reference pages |
...
| **Task-Oriented** | 12 Fast Starts | Focused guides: "Build X in 30 minutes" |
| **Full Framework** | 14 Guides | Full framework capabilities, patterns, best practices |
...
This section evaluates how KernDX aligns with the [Salesforce Well-Architected Framework](https://architect.salesforce.com/well-architected/overview) — Salesforce's prescriptive guidance for building solutions across the Trusted, Easy, and Adaptable pillars. This content is suitable for Architecture Review Boards and executive-level discussions.
...
│ EASY        │ Engaging     │ ⚠️ PARTIAL │ 53 LWC components; UI/UX out of scope    │
...
| Prevent data exposure | `MaskingRule__mdt` + `MaskingTarget__mdt` — runtime field redaction framework intercepting before-insert / before-update / before-publish on any SObject or platform event (masks via regex, Luhn for credit-card detection, literal-string, and JSON-key matching; four masking modes; three failure actions; 14 built-in masking rules; caller-class scoping). Sensitive content is rewritten in memory so the database never stores the raw value — redaction happens before persistence, distinct from `stripInaccessible` which filters after query. Ships with 14 masking-rule records (2 active by default plus 12 inactive templates) and eight masking-target records wildcarded onto four high-risk package objects; admins extend coverage to their own org's objects via additional masking-target records. **Per-SObject opt-in is a deliberate design trade-off** — most subscribers do not push credit-card data through every log emit, and a default-on regex tax on every persistence path is paid by every subscriber regardless. Subscribers prioritising default-on out-of-the-box compliance hygiene with no configuration mix `nebula-logger` alongside KernDX for log-emit masking; subscribers prioritising per-SObject performance control plus extensibility to subscriber objects ship KernDX. | default-on at the kill-switch level; per-SObject opt-in via `TriggerSetting.ApplyMasking__c` |
...
| Discoverable architecture | 26 developer documents (14 guides, 12 fast starts), 263 API references; `AGENTS.md` + `docs/Code Conventions - Guide.md` (repo root) + [`AI Agent Instructions`](AI%20Agent%20Instructions.md) (framework reference) for AI-assisted development |
| Metadata-driven behaviour | 14 custom metadata types controlling triggers, validation, feature flags, web services, mocking, masking, async jobs, field sets |
...
| Enforce quality gates | 100% per-file Apex coverage gate; PMD compliance; no inline SOQL; no `System.debug` |
...
| Configuration over code | 14 custom metadata types; `ClassTypeResolver__mdt` for type resolution |
...
| **Design Authority** | 53 LWC components, `ComponentBuilder` pattern | UI/UX design, accessibility audits, user research |
...
| **Security Operations** | Runtime data masking (any SObject), session encryption, permission enforcement | Penetration testing, threat modeling, incident response procedures |
...
| Framework-internal system-mode selectors (auxiliary read paths) | Low | `QRY_Builder` and `DML_Builder` enforce FLS/CRUD by default. About three-quarters of the 42 selector classes opt back into system-mode via a documented per-selector hook — declarations are calls in source, auditable via git + code review, and enforced by a CI-blocking scanner rule. Subscribers extending `SEL_Base` inherit FLS-enforced reads by default. Metadata kill-switches are emergency reversion without a code deploy. |
...
║   Engaging                     │ ⚠️  PARTIAL       │ 53 LWC components; UI/UX    ║
║                                │                   │ out of scope                 ║
╠════════════════════════════════╪═══════════════════╪══════════════════════════════╣
║ ADAPTABLE                      │                   │                              ║
...
**Trigger framework comparison.** `taf` and KernDX both ship declarative trigger registration. KernDX additionally ships bypass-audit emission that `taf` lacks — `taf`'s programmatic bypass surface writes no audit log. `rflib` also ships broad Trigger Framework coverage and matches KernDX on two pieces: recursion-control and per-event ordering. `taf` carries a longer accumulated tagged-release history; KernDX is v1.0 just packaged with one external client engagement at the snapshot date.

**What closes the adoption-activity gap.** As production references accumulate over time — closed external user issues, downstream consumer adoption, or third-party references — adoption activity grows. Subscribers evaluating frameworks on capability-level readiness (security defaults, code quality, test depth, capability breadth, integration coherence) should weight broad capability coverage (see [Overview § Key Findings](Strategic%20Guide%20-%20Overview.md#key-findings)) over adoption activity. Subscribers evaluating frameworks on "history of being used by other orgs" should weight the libraries that have accumulated that history accordingly.
...
1. **Source publicly available under BSL 1.1** — clients (and any other adopter) can maintain independently from the public repository; consulting engagements include direct source delivery and handover support
...
5. **Named secondary maintainer** or documented ownership of `AGENTS.md`, `docs/Code Conventions - Guide.md`, and the [`AI Agent Instructions`](AI%20Agent%20Instructions.md) framework reference within the adopting organisation
