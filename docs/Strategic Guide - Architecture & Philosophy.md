...
| **Newly released, short public adoption history**        | First validated build packaged; testing-hardened (100% per-file Apex coverage gate, 95% LWC, 561 anon-Apex assertions in the subscriber e2e harness); publicly released under BSL 1.1 and promoted for production install, in active use at one known external client engagement at the snapshot date, with no named external production deployments yet and public adoption still early. See [Adoption Signal Profile](#adoption-signal-profile) for what grows this track record, and [Metrics — Activity Snapshot](Strategic%20Guide%20-%20Metrics.md#activity-snapshot) for the current build identifier.                                                                                                                                                                                                                             |
...
| **Per-builder bypass audit emission ships, with real limits**   | Every `TRG_Base.bypass*()` mutation writes an audit log. KernDX is one of two frameworks (alongside [`rflib`](https://github.com/j-fischer/rflib)) shipping built-in trigger bypass-audit emission (`bypass_audit_signal = event`). The per-builder toggles (`QRY_Builder.withSystemMode()`, `QRY_Builder.withoutSecurity()`, `QRY_Builder.bypassSharing()`, `DML_Builder.withSystemMode()`, `DML_Builder.bypassSharing()`) also emit runtime audit entries: when the builder actually runs, it writes one entry per bypass per touched object, carrying the caller, the operation and, for DML, the submitted row count (query entries record no row count). The real limits: every bypass-audit surface is gated on the `BypassAudit_Enabled` feature flag, which one supported configuration (a hierarchy custom setting strategy) lets an administrator flip at runtime without a deployment, and switching it off is not itself recorded by the framework (at most the platform's Setup Audit Trail shows the change); framework-internal reads and writes use un-audited system-mode paths by design; and a transaction that suspends log saving and never resumes silently drops its buffered audit entries.                                                                                                                                                                                                        |
...
| Audit bypass usage            | Every trigger-bypass call writes a structured audit log carrying the correlation ID so bypasses are traceable across triggers, async chains, and API calls (`bypass_audit_signal = event`; bypass works at four levels: per-object, per-action, per-flow, framework-wide). KernDX is one of two frameworks (alongside `rflib`) shipping built-in trigger bypass-audit emission. Per-builder toggles (`QRY_Builder.withSystemMode()`, `DML_Builder.bypassSharing()`) emit the same runtime audit entries when the builder actually runs, with the object, the operation and, for DML, the submitted row count. All bypass-audit emission is gated on the `BypassAudit_Enabled` feature flag; the flag is default-on, but switching it off is not itself audited by the framework.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | default-on across the trigger, query, DML and validation surfaces                                                                                                                 |

**Security posture.** Out of the box, KernDX enforces FLS and CRUD permissions by default on both queries and DML; writes a bypass audit log across the trigger, query, DML and validation surfaces; rejects an inbound request that replays an old one with changed content (it compares a hash of the body and returns HTTP 409, a capability KernDX uniquely ships among those surveyed); and blocks any build whose code leaves an access mode undeclared. For framework-internal read and write paths, the documented opt-outs are internal system-mode hooks that write no audit entry. The emergency reversion is a metadata kill-switch (a master off-switch you flip in one record, with no code deploy, if you hit unexpected permission blocks). Encryption, sharing at the class-declaration layer, and masking all stay default-on wherever they ship.
...
| Bypass audit can be switched off without leaving its own trace  | Low    | KernDX ships bypass-audit emission across the trigger, query, DML and validation surfaces, including the per-builder toggles. The whole trail is gated on the `BypassAudit_Enabled` feature flag: default-on, but one supported configuration allows a runtime flip with no deployment, and when the flag is off nothing in the framework records that emission stopped (the platform's Setup Audit Trail is the only witness to the flag change). Framework-internal reads and writes use un-audited system-mode paths by design.                                                                                                                                                                  |
...
KernDX's first validated build is packaged and testing-hardened (a 100% per-file Apex coverage gate, 95% LWC, 561 anonymous-Apex assertions in the [subscriber e2e harness](Strategic%20Guide%20-%20Metrics.md#release-testing), 190 `@IsTest` methods across 41 subscriber test classes, an extended load suite, rolling performance baselines, and a drift-audit cycle). The honest caveat on adoption history: KernDX is publicly released under BSL 1.1 and promoted for production install, in active use at one known external client engagement at the snapshot date, with no named external production deployments yet and public adoption still early. For the current build identifier and activity counts, see [Metrics — Activity Snapshot](Strategic%20Guide%20-%20Metrics.md#activity-snapshot).
