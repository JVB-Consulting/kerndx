...
14. [Best Practices](#best-practices)
    - [1. Use ComponentBuilder for New Components](#1-use-componentbuilder-for-new-components)
    - [2. Use Utility Modules for Common Operations](#2-use-utility-modules-for-common-operations)
    - [3. Correlate Client and Server Logs](#3-correlate-client-and-server-logs)
    - [4. Handle Errors Consistently](#4-handle-errors-consistently)
    - [5. Clean Up Subscriptions](#5-clean-up-subscriptions)
    - [6. Add JSDoc to Component Classes and Methods](#6-add-jsdoc-to-component-classes-and-methods)
...
17. [Appendix: Droppable Components Quick Reference](#appendix-droppable-components-quick-reference)
    ...
    - [Complete Component Inventory](#complete-component-inventory)
      ...
      - [Subscriber-Usable Components (14 components)](#subscriber-usable-components-14-components)
...
KernDX provides a comprehensive suite of [Lightning Web Components](https://developer.salesforce.com/docs/platform/lwc/guide) designed to accelerate development while maintaining enterprise-grade patterns. The framework includes utility modules, base classes for component inheritance, and pre-built components for common use cases.
...
> **LWC Framework Scope:** 52 components across 7 utility modules, base architecture, Flow screen components, page components, form components,
> and admin components. Backed by 54 Jest test files containing ~2,268 test cases (~31K lines of test code) at 100% coverage.
...
[`docs/Code Conventions - Guide.md`](./Code%20Conventions%20-%20Guide.md). Every component requires a `.js-meta.xml` file with `apiVersion` 66.0.
...
The server automatically sets the correlation context to link client and Apex logs via [`LOG_Builder`](reference/apex/LOG_Builder.md), ensuring LWC logs appear in `LogEntry__c` with proper correlation and context.
...
Comprehensive real-time streaming event monitor for [Platform Events](https://developer.salesforce.com/docs/atlas.en-us.platform_events.meta/platform_events/platform_events_intro.htm), CDC, PushTopic, and Generic events.
...
Runs 5 diagnostic checks to verify post-install configuration and operational health.
...
### 1. Use ComponentBuilder for New Components
```javascript
// Prefer this
export default class MyComponent extends ComponentBuilder('notification', 'controller') {}
...
### 2. Use Utility Modules for Common Operations
```javascript
// Prefer this
import {formatTemplateString} from 'c/utilityString';
...
### 3. Correlate Client and Server Logs
```javascript
async saveRecord()
{
...
### 6. Add JSDoc to Component Classes and Methods
...
**Purpose:** Post-install diagnostics with 5 health checks and actionable warnings
...
**Preview Section:**
Always-visible preview showing the monospace cron string and a crontab.guru-style human-readable description (e.g., `0 30 9 ? * MON,WED,FRI` displays as "Every Monday, Wednesday, and Friday at 9:30 AM").
...
All 52 LWC components in the KernDX framework with their category, exposure status, and test coverage.
...
#### Subscriber-Usable Components (14 components)
...
> **Note:** All 52 LWC components have Jest unit tests. Run `npm run test:unit -- --coverage` to verify coverage.
