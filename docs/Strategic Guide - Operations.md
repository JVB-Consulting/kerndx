...
|----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| `TRG_Dispatcher` + `TRG_Base`                      | [`taf`](https://github.com/mitchspano/apex-trigger-actions-framework) (Apache 2.0; declarative trigger framework)   |    Medium (metadata rename plus an interface change). Both ship Change Data Capture dispatch and post-trigger finalizers. KernDX additionally records an audit entry every time a trigger is bypassed, which `taf` does not do on its programmatic surface. |
...
There are two schools of thought on how to test Salesforce code, and `fflib` and KernDX sit on different sides of it. `fflib` leans toward fast unit tests that isolate your logic
...
most. **Scaling note:** Once your test suite grows large enough that `RunLocalTests` takes longer than your team's deployment target allows, shift the ratio toward mocked tests
