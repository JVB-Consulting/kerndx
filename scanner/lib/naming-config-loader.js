...
const HARDCODED_DEFAULTS = Object.freeze({
	domains: Object.freeze(['SLS', 'ORD', 'PRD', 'SVC', 'SUB', 'MKT', 'CMN']),
	brands: Object.freeze(['ACM', 'BTA']),
	flow_types: Object.freeze(['BS', 'AS', 'BD', 'SCR', 'AL', 'SCH', 'PE', 'SF']),
	apex_layers: Object.freeze(['SEL', 'TRG', 'FLOW', 'SVC', 'BATCH', 'SCHED', 'API', 'REST', 'DTO', 'CTRL', 'UTIL', 'LOG', 'IF', 'QRY', 'DML', 'MAP', 'TST']),
...
function unfreeze(value) {
	...
	if (value && typeof value === 'object') {
		const out = {};
		for (const k of Object.keys(value)) out[k] = unfreeze(value[k]);
		return out;
	}
	return value;
}
...
function loadConfig({ repoRoot, configPath } = {}) {
	...
	if (!fs.existsSync(resolvedConfigPath)) {
		return {
			config: unfreeze(HARDCODED_DEFAULTS),
			source: 'defaults',
			configPath: null
		};
	}
	...
	try {
		...
		throw new ConfigError(
			`${resolvedConfigPath} exists but the 'js-yaml' package is not installed. ` +
	...
	if (namingBlock.domains !== undefined) ensureArrayOfStrings(resolvedConfigPath, 'domains', namingBlock.domains);
	if (namingBlock.brands !== undefined) ensureArrayOfStrings(resolvedConfigPath, 'brands', namingBlock.brands);
	if (namingBlock.flow_types !== undefined) ensureArrayOfStrings(resolvedConfigPath, 'flow_types', namingBlock.flow_types);
	if (namingBlock.apex_layers !== undefined) ensureArrayOfStrings(resolvedConfigPath, 'apex_layers', namingBlock.apex_layers);
	if (namingBlock.length_limits !== undefined && (namingBlock.length_limits === null || typeof namingBlock.length_limits !== 'object' || Array.isArray(namingBlock.length_limits))) {
		throw new ConfigError(`${resolvedConfigPath}: 'naming.length_limits' must be a YAML object (got ${Array.isArray(namingBlock.length_limits) ? 'array' : typeof namingBlock.length_limits})`);
	}
