...
test('no-coverage-exempt-without-reason', () =>
{
	ruleTester.run('no-coverage-exempt-without-reason', rule, {
		valid: [
			{
				code: '// kern-coverage-exempt: platform limit on async callback timing\nconst value = 1;', name: 'valid exemption with sufficient platform-limitation reason'
			},
			{
				code: '// regular comment\nconst value = 1;', name: 'unrelated comment is ignored'
			},
			{
				code: '// kern-coverage-exempt: unreachable under System.LimitException only\nconst value = 1;', name: 'valid exemption citing System.LimitException'
			},
			{
				code: '/* kern-coverage-exempt: block comments are not matched */\nconst value = 1;', name: 'block comments are not matched (line rule only)'
			},
			{
				code: '// kern-coverage-exempt: LMS adapter mock is async, covers branch\nconst value = 1;', name: 'reason exceeding 15 chars passes'
			}
		],

		invalid: [
			{
				code: '// kern-coverage-exempt:\nconst value = 1;', name: 'empty reason is flagged', errors: [{messageId: 'empty'}]
			},
			{
			...
			{
				code: '//   kern-coverage-exempt   :   \nconst value = 1;', name: 'loose whitespace with empty reason is flagged', errors: [{messageId: 'empty'}]
			}
		]
	});
});
