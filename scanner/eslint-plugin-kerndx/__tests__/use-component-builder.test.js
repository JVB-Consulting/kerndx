...
test('use-component-builder', () =>
{
	ruleTester.run('use-component-builder', rule, {
		valid: [
			...
			{
				code: 'class MyHelper {}', name: 'class without superclass is allowed'
			},
			{
				code: 'class MyChild extends MyParent {}', name: 'class extending non-LightningElement is allowed'
			},
			{
				code: `
		...
		invalid: [
			{
				...
				`, name: 'direct LightningElement extension is caught', errors: [{messageId: 'useCB'}]
			},
			{
				code: `
					import {LightningElement, api} from 'lwc';
					export default class MyComponent extends LightningElement {}
				`, name: 'LightningElement with other named imports is caught', errors: [{messageId: 'useCB'}]
			},
			{
				code: `
					import {LightningElement as BaseElement} from 'lwc';
					export default class MyComponent extends BaseElement {}
				`, name: 'aliased LightningElement import is caught', errors: [{messageId: 'useCB'}]
			},
			{
				code: `
					import {LightningElement as LE} from 'lwc';
					export default class MyComponent extends LE {}
				`, name: 'short alias of LightningElement is caught', errors: [{messageId: 'useCB'}]
			},
			{
				code: `
					import {LightningElement as Base, api} from 'lwc';
					export default class MyComponent extends Base {}
				`, name: 'aliased LightningElement with other named imports is caught', errors: [{messageId: 'useCB'}]
			}
		]
	});
});
