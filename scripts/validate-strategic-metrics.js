...
function countLwcDirs(repoRoot)
{
	...
	if(!fs.existsSync(lwcRoot)) return 0;
...
function countLineStartIsTestMethods(repoRoot)
{
	...
	{
		if(!entry.endsWith('_TEST.cls')) continue;
		const text = fs.readFileSync(path.join(dir, entry), 'utf-8');
		const lines = text.split('\n');
		for(const line of lines)
		{
			const trimmed = line.trimStart();
			if(!trimmed.startsWith('@IsTest')) continue;
			if(trimmed.includes('(') || trimmed.includes('class ')) continue;
			count++;
		}
	}
	return count;
...
function computeExpected(repoRoot)
{
	...
	const fastStartGuides = fs.existsSync(docsDir)
		? fs.readdirSync(docsDir).filter(f => f.startsWith('Fast Start - ') && f.endsWith('.md')).length
		: 0;
	const developerGuides = fs.existsSync(docsDir)
		? fs.readdirSync(docsDir).filter(f => f.endsWith(' - Guide.md')).length
		: 0;
	const strategicGuidesSource = fs.existsSync(docsDir)
		? fs.readdirSync(docsDir).filter(f => f.startsWith('Strategic Guide - ') && f.endsWith('.md')).length
		: 0;
	...
	const strategicGuidesShipped = fs.existsSync(docsDir)
		? fs.readdirSync(docsDir).filter(f =>
			f.startsWith('Strategic Guide - ')
			&& f.endsWith('.md')
			&& !INTERNAL_ONLY_GUIDE_MARKERS.some(marker => f.includes(marker))
		).length
		: 0;

	const apexReference = fs.existsSync(path.join(referenceDir, 'apex'))
		? fs.readdirSync(path.join(referenceDir, 'apex')).filter(f => f.endsWith('.md')).length
		: 0;
	const metadataReference = fs.existsSync(path.join(referenceDir, 'metadata'))
		? fs.readdirSync(path.join(referenceDir, 'metadata')).filter(f => f.endsWith('.md')).length
		: 0;
	const objectsReference = fs.existsSync(path.join(referenceDir, 'objects'))
		? fs.readdirSync(path.join(referenceDir, 'objects')).filter(f => f.endsWith('.md')).length
		: 0;
	const eventsReference = fs.existsSync(path.join(referenceDir, 'events'))
		? fs.readdirSync(path.join(referenceDir, 'events')).filter(f => f.endsWith('.md')).length
		: 0;
	...
	{
		if(!fs.existsSync(releaseTestingClassesDir)) return 0;
		let count = 0;
...
function parseDocClaims(content)
{
	...
	{
		if(!claims.has(key)) claims.set(key, {value, line});
	}
	...
	for(let i = 0; i < lines.length; i++)
	{
		...
		if(labelLc === 'apex classes (total)') setIfMissing('apexTotal', value, i + 1);
		if(labelLc === 'apex production classes') setIfMissing('apexProduction', value, i + 1);
		if(labelLc === 'apex test classes') setIfMissing('apexTest', value, i + 1);
		if(labelLc === 'apex test methods') setIfMissing('apexTestMethods', value, i + 1);
		if(labelLc === 'lwc components (total)') setIfMissing('lwcDirs', value, i + 1);
		if(labelLc === 'jest test files') setIfMissing('lwcTestFiles', value, i + 1);
		if(labelLc === 'developer guides') setIfMissing('developerGuides', value, i + 1);
		if(labelLc === 'fast start guides') setIfMissing('fastStartGuides', value, i + 1);
		if(labelLc === 'strategic guide documents') setIfMissing('strategicGuidesShipped', value, i + 1);
		if(labelLc === 'api reference pages (apex)') setIfMissing('apexReference', value, i + 1);
		if(labelLc === 'api reference pages (metadata)') setIfMissing('metadataReference', value, i + 1);
		if(labelLc === 'api reference pages (objects)') setIfMissing('objectsReference', value, i + 1);
		if(labelLc === 'api reference pages (events)') setIfMissing('eventsReference', value, i + 1);
		if(labelLc === 'api reference pages (total)') setIfMissing('referenceTotal', value, i + 1);
		if(labelLc === 'total documentation files (developer-focused)') setIfMissing('totalDocFiles', value, i + 1);
		if(labelLc === 'subscriber apex classes (production)') setIfMissing('subscriberApexProduction', value, i + 1);
		if(labelLc === 'subscriber test classes') setIfMissing('subscriberApexTest', value, i + 1);
		if(labelLc === 'subscriber test methods') setIfMissing('subscriberTestMethods', value, i + 1);
		if(labelLc === 'spec files') setIfMissing('e2eSpecFiles', value, i + 1);
	}
	return claims;
}
