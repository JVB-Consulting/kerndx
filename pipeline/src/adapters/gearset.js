...
function build(_adapterConfig, branchesConfig) {
	...
	return {
		...
		classifyHeadRef(headRef) {
			if (backPromRe.test(headRef)) return { action: 'skip-scan', label: 'back-promotion' };
			if (featureRe.test(headRef)) return { action: 'full-scan', label: 'intercepted-feature' };
			if (headRef.startsWith('gs-pipeline/')) return { action: 'full-scan', label: 'suspicious-prefix' };
