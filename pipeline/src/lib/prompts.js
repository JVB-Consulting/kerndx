...
function readlineSession({stdin, stdout})
{
	...
	return {
		askText: async(prompt) => (await ask(prompt)).trim(), askChoice: async(prompt, choices) =>
		{
...
async function askText(prompt, opts = {})
{
	...
	try
	{
		return await session.askText(prompt);
	}
	finally
	{
		session.close();
...
async function askChoice(prompt, choices, opts = {})
{
	...
	try
	{
		return await session.askChoice(prompt, choices);
	}
	finally
	{
		session.close();
...
async function askYesNo(prompt, defaultYes = true, opts = {})
{
	...
	try
	{
		return await session.askYesNo(prompt, defaultYes);
	}
	finally
	{
		session.close();
