...
function extractSelectorsFromFile(filePath)
{
	...
	{
		return {filePath, found: [], error: `not found: ${filePath}`};
	}
...
function checkParity(sources)
{
	...
	for(const r of results)
	{
		if(r.error)
		{
			issues.push(`${r.label} (${r.filePath}): ${r.error}`);
		}
		else if(r.found.length === 0)
		{
			issues.push(`${r.label} (${r.filePath}): no --rule-selector found`);
		}
	}
	if(issues.length)
	{
		return {ok: false, issues, results};
	}
