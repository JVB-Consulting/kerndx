...
function deriveTemplateData(config)
{
	...
	const lengthLimit = (naming.length_limits && naming.length_limits.apex !== undefined) ? naming.length_limits.apex : DEFAULT_APEX_LENGTH;
...
function renderPmdRuleset(config, {templatePath = TEMPLATE_PATH} = {})
{
	...
	const eta = new Eta({
		views: path.dirname(templatePath), cache: false, autoEscape: false, useWith: true, tags: [
			'<%',
			'%>'
		], autoTrim: false
	});
