...
function groupRows(rows)
{
	...
	for(const r of rows)
	{
		...
		if(!map.has(key))
		{
			map.set(key, {sev: r.sev, src: r.src, rule: r.rule, fname: r.fname, lines: [], cnt: 0});
		}
	...
	for(const entry of map.values())
	{
		...
		{
			lineStr = `${uniq[0]},…,${uniq[uniq.length - 1]}`;
		}
...
function buildSlackPayload(opts)
{
	...
	try
	{
		...
		const col = (row, ...names) =>
		{
			...
			{
				const i = hIdx[n];
				if(i !== undefined && row[i] !== undefined)
				{
					return (row[i] || '').trim();
				}
			}
	...
	if(blockingGrouped.length > 0)
	{
		...
		const hdrLine = padEnd('SEV', C_SEV) + '  ' + padEnd('SRC', C_SRC) + '  ' + padEnd('RULE', C_RULE) + '  ' + padEnd('FILE', C_FILE) + '  ' + padStart('LINE(S)', C_LINE)
				+ '  ' + padStart('CNT', C_CNT);
		...
		{
			tableLines.push(
					padEnd(sev, C_SEV) + '  ' + padEnd(src.slice(0, C_SRC), C_SRC) + '  ' + padEnd(truncMid(rule, C_RULE), C_RULE) + '  ' + padEnd(truncMid(fname, C_FILE), C_FILE)
					+ '  ' + padStart(lineStr.slice(0, C_LINE), C_LINE) + '  ' + padStart(String(cnt), C_CNT));
		}
		if(overflow > 0)
		{
			tableLines.push(`...and ${overflow} more blocking violation(s); full list in the artifact`);
		}
		blockingText = '```\n' + tableLines.join('\n') + '\n```';
	}
	...
	if(topText)
	{
		fields.push({title: 'Top blocker', value: topText, short: false});
	}
	if(blockingText)
	{
		fields.push({title: 'Blocking violations (Sev-1/Sev-2)', value: blockingText, short: false});
	}
	if(infoText)
	{
		fields.push({title: 'Informational', value: infoText, short: false});
	}
	...
	return {
		text: `${heading} SFCA ${status} on <${prUrl}|PR #${prNumber}> — ${totalV} violation(s)`, attachments: [
			{
				color, title, title_link: prUrl, fields, mrkdwn_in: [
					'text',
					'fields'
				]
			}
...
function slackPayload(opts)
{
	...
	if(payload === null)
	{
		return JSON.stringify({should_notify: false}, null, 2);
	}
