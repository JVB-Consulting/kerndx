...
{
	assert.deepEqual(adapter.classifyHeadRef('promotion/P0001-DeployToUAT'), {action: 'skip-scan', label: 'back-promotion'});
});

test('promotion to production also matches', () =>
{
	assert.deepEqual(adapter.classifyHeadRef('promotion/P0042-DeployToProduction'), {action: 'skip-scan', label: 'back-promotion'});
});

test('back-promotion to Dev (forward-and-back use same pattern in Copado)', () =>
{
	assert.deepEqual(adapter.classifyHeadRef('promotion/P0019-DeployToDev'), {action: 'skip-scan', label: 'back-promotion'});
});

test('feature branch unaffected', () =>
{
	assert.deepEqual(adapter.classifyHeadRef('feature/US-0000123'), {action: 'full-scan', label: 'normal'});
});
...
test('plan\'s hypothetical cmc/promote pattern is NOT a back-promotion (regression-guard)', () =>
{
	...
	assert.deepEqual(adapter.classifyHeadRef('cmc/promote-dev-to-uat'), {action: 'full-scan', label: 'normal'});
});
