...
test('autorabit returns full-scan/normal for every ref (verified-as-none per Phase 0.3)', () => {
	...
	assert.deepEqual(adapter.classifyHeadRef('main'),
		{ action: 'full-scan', label: 'normal' });
	assert.deepEqual(adapter.classifyHeadRef('feature/my-thing'),
		{ action: 'full-scan', label: 'normal' });
	assert.deepEqual(adapter.classifyHeadRef('autorabit/promotion-anything'),
		{ action: 'full-scan', label: 'normal' });
});
...
test("plan's speculative autorabit/promotion-* pattern is NOT a back-promotion (regression-guard)", () => {
	...
	assert.deepEqual(adapter.classifyHeadRef('autorabit/promotion-build-to-uat'),
		{ action: 'full-scan', label: 'normal' });
	assert.deepEqual(adapter.classifyHeadRef('cab-release-2026-05'),
		{ action: 'full-scan', label: 'normal' });
});
