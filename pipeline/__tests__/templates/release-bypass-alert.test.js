...
test('release-bypass-alert uses runsOn value', () =>
{
	const rendered = renderTemplate(TPL, {
		releaseBranches: [
			...
			'uat',
			'main'
		], requiredChecks: ['Static Code Analysis'], runsOn: 'self-hosted', nodeVersion: '20', slack: {enabled: true, webhook_env_var: 'MY_WEBHOOK'}
	}, {views: path.dirname(TPL)});
...
test('release-bypass-alert uses reviewDecision check not check-runs', () =>
{
	const rendered = renderTemplate(TPL, {
		releaseBranches: [
			'build',
			'uat'
		], requiredChecks: ['Static Code Analysis'], runsOn: 'ubuntu-latest', nodeVersion: '20', slack: {enabled: true, webhook_env_var: 'SLACK_WEBHOOK_URL'}
	}, {views: path.dirname(TPL)});
