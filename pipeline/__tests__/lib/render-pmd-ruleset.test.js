...
const {
	renderPmdRuleset, deriveTemplateData, PmdRulesetError, DEFAULT_DOMAINS, DEFAULT_BRANDS, DEFAULT_LAYERS, DEFAULT_APEX_LENGTH, TEMPLATE_PATH
...
test('renderPmdRuleset with empty config renders defaults byte-identical to checked-in XML', () =>
{
	...
	assert.equal(rendered, checkedIn,
			'Rendered output drifted from scanner/subscriber-naming-pmd-ruleset.xml. ' + 'Either the template changed without updating the checked-in XML, or the '
...
{
	assert.deepEqual(DEFAULT_DOMAINS, [
		'SLS',
		'ORD',
		'PRD',
		'SVC',
...
{
	assert.throws(() => renderPmdRuleset({
		naming: {
			domains: [
				'inv',
				'WMS'
...
{
	assert.throws(() => renderPmdRuleset({naming: {domains: 'INV,WMS'}}), (err) => err instanceof PmdRulesetError && /must be an array/.test(err.message));
});

test('renderPmdRuleset rejects length_limit below 10', () =>
{
	assert.throws(() => renderPmdRuleset({naming: {length_limits: {apex: 5}}}), (err) => err instanceof PmdRulesetError && /between 10 and 80/.test(err.message));
});

test('renderPmdRuleset rejects length_limit above 80', () =>
{
	assert.throws(() => renderPmdRuleset({naming: {length_limits: {apex: 100}}}), (err) => err instanceof PmdRulesetError && /between 10 and 80/.test(err.message));
});
...
test('renderPmdRuleset ignores naming.enabled and other unrelated config fields', () =>
{
	const rendered = renderPmdRuleset({
		naming: {
			enabled: true, domains: ['INV'], field_prefix_rule: 'cross-domain-only', truncations: ['Mgmt -> Mgt'], suppressions_file: '.kerndx/naming-suppressions.json'
		}, ci_adapter: {name: 'gearset'}, package_dirs: ['force-app/main/default']
	});
