...
| `LOG_Builder`                                      | [`nebula-logger`](https://github.com/jongpie/NebulaLogger) (ships historical log browser + retention engine)        |                                                                                                                                Low (similar platform-event pattern). `nebula-logger` goes further here on retention overrides and Big Object archival; both ship a screen for browsing past log entries, where KernDX's Log Console adds grouped recurring problems and a per-operation drilldown. |
...
materials are the full source under the BSL 1.1 license, 21 guides, 16 Fast Starts, 269 API reference pages, 100% per-file Apex test coverage that acts as a safety net against
...
A common concern is that installing KernDX deploys all [190 production classes](Strategic%20Guide%20-%20Metrics.md#package-codebase) even if all you need is triggers. If KernDX shipped as source code you deploy yourself, that would
